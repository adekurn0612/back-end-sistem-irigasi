...
const dataKalium = async (req, res) => {
    try {
        const query =
            ...
            FROM irigasi
            WHERE timestamp >= DATE_SUB((SELECT MAX(timestamp) FROM irigasi), INTERVAL 7 DAY) AND nama = 'Lora 1'
...
const dataKaliumTerbaru = async (req, res) => {
    try {
        const query =
            `SELECT sensor_k FROM irigasi where nama = "Lora 1"
...
const dataNitrogenTerbaru = async (req , res)=>{
    try{
        const query = 
        `SELECT sensor_n FROM irigasi where nama = "Lora 1"
        ORDER BY timestamp DESC 
        LIMIT 1;
        `;
        const result = await sequelize.query(query);
...
const dataPHTerbaru = async (req , res)=>{
    try{
    const query = 
        `SELECT sensor_ph FROM irigasi where nama = "Lora 1"
        ORDER BY timestamp DESC 
        LIMIT 1;
        `;
    const result = await sequelize.query(query);
...
const dataPotasiumTerbaru = async (req, res)=>{
    try{
        const query = 
        `SELECT sensor_p FROM irigasi where nama = "Lora 1"
        ORDER BY timestamp DESC 
        LIMIT 1;
        `;
        const result = await sequelize.query(query);
...
const dataKelembapanTerbaru = async (req, res) => {
    try{
        const query = 
        `SELECT sensor_kelembaban FROM irigasi where nama = "Lora 1"
        ORDER BY timestamp DESC 
        LIMIT 1;
        `;
        const result = await sequelize.query(query);
