...
const dataKaliumTerbaru = async (req, res) => {
    try {
        const query =
            `SELECT sensor_k FROM coba where nama = "Lora 2"
...
const dataNitrogenTerbaru = async (req , res)=>{
    try{
        const query = 
        `SELECT sensor_n FROM coba where nama = "Lora 2"
        ORDER BY timestamp DESC 
        LIMIT 1;
        `;
        const result = await sequelize.query(query);
...
const dataPHTerbaru = async (req , res)=>{
    try{
    const query = 
        `SELECT sensor_ph FROM coba where nama = "Lora 2"
        ORDER BY timestamp DESC 
        LIMIT 1;
        `;
    const result = await sequelize.query(query);
...
const dataPotasiumTerbaru = async (req, res)=>{
    try{
        const query = 
        `SELECT sensor_p FROM coba where nama = "Lora 2"
        ORDER BY timestamp DESC 
        LIMIT 1;
        `;
        const result = await sequelize.query(query);
...
const dataKelembapanTerbaru = async (req, res) => {
    try{
        const query = 
        `SELECT sensor_kelembaban FROM coba where nama = "Lora 2"
        ORDER BY timestamp DESC 
        LIMIT 1;
        `;
        const result = await sequelize.query(query);
